   .data
        ...
        testResultStringg: .space 256  # Adjust the size as needed #second output string

        #################################################################################################3
        ###
        #1201139
        #1200071
        INTpart: .word 0       # Space for the integer part
        fracPart: .word 0    # Space for the fractional part as an integer
        TENpower: .word 1             # TENpower
        resultString: .space 12 #buffer to save the float results  
        counter: .float 7.0  # Testing 
        MSG1: .asciiz "Converted Float: "
        eachFLOAT: .asciiz "EACH Float: \n"
        ACCUMFLOAT: .asciiz "ACCUMFLOAT: \n"

        testResultString: .space 50
        second_testResultString: .space 50 #to save the second reading of BPT result

        messageeHgb: .asciiz "Average Hgb: "
        messageeBGT: .asciiz "Average BGT: "
        messageeLDL: .asciiz "Average LDL: "
        messageeBPT: .asciiz "Average BPT: \nDiastolic AVG: "
        messageSystolic: .asciiz "\nSystolic AVG:  "


        zerooFloat: .float 0.0   # Define a floating point zero constant in data segment


        # for normal and unnormal test ---------------------
            HgbLowerBound: .float 13.8
            HgbUpperBound: .float 17.2
            BGTLowerBound: .float 70.0
            BGTUpperBound: .float 99.0
            upperBoundLDL: .float 100.0
            SystolicBPT: .float 120.0
            DiastolicBPT: .float 80.0
            
            EnterIDprompt: .asciiz "\nEnter the Patient ID'): "  
            enteredID: .space 11  # Buffer FOR ENTERED ID
            error_msg: .asciiz "Failed to open the file.\n"
            


            user_entered_test_name: .word 0



            found_unnormal_test: .asciiz "\nThe unnormal test is found successfully.\n"
            not_found__recordd: .asciiz "\nThe test recorde is not found in the file.\n Please try again.\n"
            found_bool: .word 0
            testResultString2: .space 50
...
enter_valid_choice:
        ...
        beq $v0, 51, searchUnormal_by_test_name
...
enter_valid_option_second_menu:
        ...
        beq $v0, 50, search_for_UNNORMAL_by_patientID
...
#--------------------------------------------------------------------------------------------------#
    #this function returns the abnormal tests according to the patiect id 
    #it starts by asking the user to enter the id and search for the ID and returns if the test is upnormal or what

#------------------------------------------------------------------------------------------------#
search_for_UNNORMAL_by_patientID:


        li $s5,0 #used for the secondery menu
        ...
        jal validate_id #this function is called in order to validate that the id is 7 digits and only digits not characters
        #in this function we read and validate the id of the ID.


        li $s7, 0 # set s7 which is the flag for ending the file to 0 at first

        la $a0, buffer #load the buffer into register a0


LOOPthroughfile:

            move $t9, $a0 # store a0 in t9 to remain its OG value.

            jal IDexistcheck # check for each line if the ID is in the file 
            # it takes a0 along to the ID entered by the user and returns 1 in t5 if there is a found ID that equals the ID entered by the user
            # it returns 0 if the ID is not found in the file
            #if the t6 = 1 mean the ID is equal to the enteredID
            #else the ID is not equal to the enteredID

            beq $t6, 1, checkTestResult #if the ID is found in the file, check the test result if unnormal or not
            jal NEXTLINEEE
            beq $s7, 1, ContorMainMenu # If the end of the file is reached, give the user the option to return to the menu or to continue
            j LOOPthroughfile


    # Hemoglobin (Hgb): 13.8 to 17.2 grams per deciliter 
    # Blood Glucose Test (BGT): Normal Range Between 70 to 99 milligrams per deciliter (mg/dL) 
    # LDL Cholesterol Low-Density Lipoprotein (LDL): Normal Range Less than 100 mg/dL
    # Blood Pressure Test (BPT): Normal Range: Systolic Blood Pressure: Less than 120 millimeters of mercury (mm Hg).
    # Diastolic Blood Pressure: Less than 80 mm Hg 

checkTestResult: 

            move $a0, $t9          # Load the address of the start of the line into $a0
            jal line_by_line_function # f(a0) retrun f1 = test result in floating point,
            # t4 = type of test
            # a0 = start of the next line

            #-----------------------------------sum the values of the test result to calculate the average--------------------------------
            
            beq $t4, 1, HgbComparison # If the test is Hemoglobin, branch to HgbCompa
            beq $t4, 2, BGTComparison # If the test is Blood Glucose Test, branch to BGTComparison
            beq $t4, 3, LDLComparison # If the test is LDL Cholesterol, branch to LDLComparison
            beq $t4, 4, BPTComparison # If the test is Blood Pressure Test, branch to BPTComparison


            HgbComparison: # If the test is Hemoglobin, branch to HgbCompa:
            
                            lwc1 $f3, HgbLowerBound # load 13.8 in f3
                            lwc1 $f4, HgbUpperBound # load 17.2 in f4

                            c.lt.s $f1, $f3         # compare f1 which has test result with f3 which has the lower bound
                            bc1t PrintTESTunnormal   # If the test result is less than the lower bound, branch to ResultUNnormal

                            c.lt.s $f4, $f1         # compare f1 which has test result with f4 which has the upper bound
                            bc1t PrintTESTunnormal   # If the test result is greater than the upper bound, branch to ResultUNnormal

                            
                            beq $s7, 1, ContorMainMenu  #If the end of the file is reached, give the user the option to return to the menu or to continue
                            # If the end of the file is not reached, continue to the next line
                            j LOOPthroughfile       # Continue to check file IDs							
                                                        
                                    
                    
            BGTComparison: # If the test is Blood Glucose Test, branch to BGTComparison:

                           lwc1 $f3, BGTLowerBound # load 70.0 in f3
                           lwc1 $f4, BGTUpperBound # load 99.0 in f4
            ...
            LDLComparison :# If the test is LDL Cholesterol, branch to LDLComparison:

 
                            lwc1 $f4, upperBoundLDL  # load 100.0 into $f4
                            c.le.s $f4, $f1          # compare f1 which has test result with f4 which has the lower bound
                            bc1t PrintTESTunnormal    # If $f1 is greater than or equal to 100 branch to PrintTESTunnormal
                           
                            beq $s7, 1, ContorMainMenu #If the end of the file is reached, give the user the option to return to the menu or to continue
                            j  LOOPthroughfile         
                        		
            BPTComparison: # If the test is Blood Pressure Test, branch to BPTComparison: 

                            lwc1 $f4, SystolicBPT # Load 120.0 in f4
                            lwc1 $f3, DiastolicBPT # Load the 80.0 in f3
              ...
              PrintTESTunnormal:
                            ...
                            move $a0, $t9           # Load the address of the start of the line into $a0
                            jal printLinee             # Jump to printLinee to print the data for this line
...
ContorMainMenu:
            # Check if v1 is 1
            beq $s5, 1, record_foundt_2  # If v0 is 1, branch to record_found
            ...
            move $s5, $v0     # Move user input to $v1

handle_inputt_2:
            beq $s5, 99, search_for_UNNORMAL_by_patientID       # If the input is 'c' (ascii value 99), branch to update_test
            beq $s5, 109, menu             # If the input is 'm' (ascii value 109), branch to menu
            ...
            syscall
            move $s5, $v0     # Move user input to $v1
            j handle_inputt_2     # Jump back to handle user input

record_foundt_2:

        # Print result updated successfully
        li $v0, 4                        # Print string syscall
        la $a0, found_unnormal_test # Load address of message
        syscall
...
handlee_inputt_2:
        ...
        syscall
        move $s5, $v0     # Move user input to $v1
        j handlee_inputt_2     # Jump back to handle user input
...
####################################
#this function is created to check if the entered ID equals the ID in each line of the file
#it rerturns 1 if id is equal to the enteredI, 0 otherwise
####################################

IDexistcheck:

            #a0 points to a line in the buffer

            move $t8, $ra # store the return address in t8
            ...
            li $t3, 0
            la $a0, patient_id
            jal calculateSum       # sum of the entered ID
            move $t6, $v0          # Move result saved in v0 to to $t6



            move $a0, $t7 #move the address of the next line to a0 again
            # Reset $t3 to 0 to reuse to compare the ID entered with the ID in the buffer
...
findIDineachLine:
            ...
            lb $a1, 0($a0)         
            beq $a1, ':', checkIdIfEqual    #when reaching the : compare the IDs
            addiu $a0, $a0, 1      # Move to the next character in buffer
            addu $t3, $t3, $a1     # Add the ASCII value to the sum for ID comparison and store in t3 the sum of the files ID
            j findIDineachLine


checkIdIfEqual:

            beq $t6, $t3, ifIDsEqual # Compare sum of ASCII values of t6 (entered ID) and t3 (ID in the file)
            # If they are not equal jump back to the next line 
            li $t6, 0
            move $ra, $t8 # restore the return address saved above
            jr $ra
...
################################################ here the checking step is done ##########################################



############################################here we calculate the sum of the entered ID###################################
...
summing_loop:
            ...
            beq $t1, '\0', sum_is_done # Check for end of string
            beq $t1, '\n', sum_is_done # Check for end of string
...
################################################ this function is to print each line     ##########################################
printLinee:

        # f(a0) -> print line 
        # t9 has the address of next line with a0 
            
        lb $a1, 0($a0)
        beq $a1, '\n', lineDonePrintingg  # End of data for this line
        beq $a1, '\0', endOFfile  # End of data for this line

        move $a0, $a1 
        li $v0, 11             # syscall for printing a character
        syscall
        
        addiu $t9, $t9, 1      # move to the next char
        move $a0, $t9          # load the address of t9 into a0
    
        j printLinee            # go back to print the next character
        
lineDonePrintingg:

        move $a0, $a1
        li $v0, 11             # syscall for printing the last character in a1
        syscall
...
NEXTLINEEE:
    # it will return in a0 the start of the next line 
    #inn  s7 = 1 if the bufffer is done and 0 otherwise

    lb $a1 , 0($a0) # Load the next character from the buffer into $a1
    beq $a1, '\0', endOFfile  # End of data for this line
    beq $a1, '\n', done_NEXTLINEEE  # End of data for this line
    addiu $a0, $a0, 1      # Move to the next character in buffer
    j NEXTLINEEE         # Continue printing data
   ...
   done_NEXTLINEEE: 
    ...
    endOFfile: #if a null terminator is found then set s7 to 1
    li $s7 , 1 # set the value of s7 to 1 to indicate that the buffer is done.
    jr $ra
...
average_test_value:
   ...
   # choosing the reg s0 to s3 to save the count of each test result
    ...
    la $a0, zerooFloat     # Load the address of the zerooFloat constant
...
#-----------------------------------Getting the floating-point values from the file--------------------------------
    ...
    la $a1, testResultString   # Load address of the output string into $a1




#line_by_line_function: 
...
# s1 = count of Hgp, s2 = count of BGT, s3 = count of LDL, s4 = count of BPT  ,in the buffer
   ...
   get_values_from_line:
            ...
            jal line_by_line_function # Jump to the line_by_line_function label

            #-----------------------------------sum the values of the test result to calculate the average--------------------------------

            beq $t4, 1, HgbResult_sum
            beq $t4, 2, BGTResult_sum
            beq $t4, 3, LDLResult_sum
            beq $t4, 4, BPTResult_sum
...
find_the_avg:
   	     ...
   	     li $v0, 4          # System call for printing a character
             la $a0, messageSystolic          # Load ASCII value of newline ('\n') into $a0
             syscall 
...
the_id_is_in_t4_part5:
            ...
            #print the line if the id = user input
...
end_programm:
 ...
 #this function returns float in f1 and the type of the test name for a definite line stored at a0

#line_by_line_function: #go back to avg

# this functionn will retrun the float value of the test result in F1
# and it returns the type of the test in t4
# and also a0 will be the start of the next line 

# t4 = 1 for Hgb, t4 = 2 for BGT, t4 = 3 for LDL, t4 = 4 for BPT
# s1 = count for Hgp, s2 = for of BGT, s3 = for of LDL, s4 = for of BPT 

line_by_line_function:

        la $a1, testResultString   # Load address of the test result as a string into a1 
        
        # .data section : testResultString: .space 50  # Allocate space for the output string

        li $t2, 0 # reset the value of asscii sum.

        move $t6, $ra # save the return address in t6


findd_semicolon_in_line:

        lb $t0, 0($a0)        # load the first byte of the a0 register into t0
        beq $t0, ':', testname_determining # when reaching the : , jump to testname_determining
        li $t5, ','           # load the asci value of the comma into t5
        beq $t0, $t5, increment_counterrr # if the char read is a comma , jump to increment_counterrr
        addiu $a0, $a0, 1     # Move to the next character in the line in a0
        j findd_semicolon_in_line      # Jump back to the start of the loop

increment_counterrr:
        addiu $t2, $t2, 1     # incremennt the counter of the comma
        addiu $a0, $a0, 1     # movw the pointer to the next char after the comma
        beq $t2, 2, startcopying_testname # If you founnd two semicolons, start copying the test name
        j findd_semicolon_in_line     # Otherwise, keep looking for semicolons

startcopying_testname:

        lb $t0, 0($a0)        # Load the next character from the input string into $t0
        beq $t0, ' ', skipTHESpace # Skip spaces
        beq $t0, '\n', ENDofLine # Check for the new line at the end of the line
        beq $t0, '\0', ENDofLine # Check for the end of the line or end of file

        sb $t0, 0($a1)        # Stoore the character in the test result string
        beq $t0, '/', BPTtest # If backslash is found, jump to BPTtest
        addiu $a0, $a0, 1     # Movee to the next character in the input string
        addiu $a1, $a1, 1     # Movee to the next position in the output string
        j startcopying_testname       # go back to the start of the copying loop

skipTHESpace:
         addiu $a0, $a0, 1     # Movee to the next character in the input string
        j startcopying_testname

#add \n after the first number in the BPT record
 BPTtest: 
             # just add \n to this byte after \ does exist
                li $t1, 0x0a          # ASCII value of '\n'is 10
                sb $t1, 0($a1)        # Add a '\n' to the output string
                addiu $a0, $a0, 1     # Move to the next character in the input string
                la $a1, testResultString2
                j startcopying_testname
...
#################################### return unique value for each test name #################################
 ...
 testname_determining: 
            li $t3, 0 # get the asci sum of the test name of the record

 GetUniqeValueOfTestName:

            addiu $a0, $a0, 1      # add a0 by 1 , go to the next character from above
            lb $t0, 0($a0)        # Load the next character from the input string into $t0
            beq $t0, ' ', GetUniqeValueOfTestName # if a space is found , skip it
            
            # calcualte the sum of the test name in the record by the ascii values of the characters

            beq $t0, ',', getTestType #If the colon is reached then you have the test name ready to be compared
            addu $t3, $t3, $t0     # Add the ASCII value to the sum for test name comparison
                
            jal GetUniqeValueOfTestName #the main return address is moved to t6 above

    
getTestType:

    addiu $t2, $t2, 1     # semicolon counter increment by 1

    #check for the asci sums of the test name to return the unique value for each test name
    li $t1, 0x111        # "Hgb" is 0x111
    beq $t3, $t1, HgbResult # If the sum matches "Hgb", jump to HgbResult

    li $t1, 0xDD         #  "BGT" is 0xDD
    beq $t3, $t1, BGTResult # If the sum matches "BGT", jump to BGTResult

    li $t1, 0xDC        #  "LDL" is 0xDC
    beq $t3, $t1, LDLResult # If the sum matches "LDL", jump to LDLResult

    li $t1, 0xE6         #  "BPT" is 0xE6
    beq $t3, $t1, BPTResult # If the sum matches "BPT", jump to BPTResult
...
###########################################################################################################
###########################################################################################################
#this is reached whenever a new line is reached or the end of the file is reached

ENDofLine:

            li $t2,0 # rest the value of t2 to 0  for string2 which is the counter for the comma
            move $t7, $a0          # save the start of the next line
            sb $t0, 0($a1)        #store the first character of the test result string in t0 register


            #check if the test result has a decimal point or not

          la $a0, testResultString
          DOTchecking:
                    lb $t0, 0($a0)        #load the next character from the test result string into t0
                    beq $t0, '.', DOT_is_found # If a dot is found, jump to DOT_is_found
                    beq $t0, '\n', NODOT # If newline is found, jump to NODOT
                    beq $t0, '\0', NODOT # If newline is found, jump to NODOT
                    
                    addiu $a0, $a0, 1      # Move to the next character in theresult test string
                    j DOTchecking            # Jump back to the start of the loop

          NODOT:
                    li $t1, 0x2E          # this is the asci value of '.'
              ...
              DOT_is_found :  
                     ...
                     #store in t2 the value 1  to not make the loop to check the dot again
                        li $t2, 1
                        la $a0, testResultString2   # Load address of the output string into $a0
                        j DOTchecking
            ...
            beq $t4, 1, HgbResult_type
            beq $t4, 2, BGTResult_type
            beq $t4, 3, LDLresult_type
            beq $t4, 4, BPTresult_type

            HgbResult_type:
                        la $a0, testResultString   # Load address of the testResultString into $a0
            ...
            BGTResult_type:
                        la $a0, testResultString   # Load address of the output string into $a0
                        jal parseeString        # Jump to the string parsing function
                        jal convertToFlaot
                        # f1 will have the float value of the test result
			            j conversion_end            
...
conversion_end:
                ...
                move $ra, $t6 # restore the return address form t6
                addiu $t7, $t7, 1 # go to the next line 
                move $a0, $t7    # save the  the next line stored in a0 
                li $t2, 0              # Reset the comma counter  for next ID
                lb $a1, 0($a0)
                beq $a1, '\0', end_of_bbuffer # Check for end of buffer
                li $s7, 0  #set s7 to 1 to indicate that the buffer is done
                jr $ra

                end_of_bbuffer:
                li $s7, 1 # set s7 to 1 to indicate that the buffer is done
                jr $ra
...
############################################################################################
# Function to parse a string to a float

parseeString:
        # Initialize variables
        li $t3, 0              # Will hold the integer part
        li $t2, 0              # Will hold the fractional part
        li $t1, 1              # Will be used for the TENpower
    
    
    # Parse the integer part
parseInteger:
        lb $t0, 0($a0)         # Load the next byte (character) from the string
        beq $t0, '.', int_done # Check for decimal point    
        beq $t0, '\0', no_value_to_parsee # Check for null terminator
        sub $t0, $t0, '0'      # Convert from ASCII to integer
        mul $t3, $t3, 10       # Multiply current result by 10
        add $t3, $t3, $t0      # Add the new digit
        
        addiu $a0, $a0, 1      # Move to the next character
        j parseInteger         # Loop back

int_done:
        sw $t3, INTpart    # Store the integer part
        addiu $a0, $a0, 1      # Move past the decimal point

    # Parse the fractional part
parseFractional:
        lb $t0, 0($a0)         # Load the next byte (character)
        beq $t0, '\n', endFractional # Check for new line terminator
        ...
        mul $t1, $t1, 10       # Increase TENpower #t3
        addiu $a0, $a0, 1      # Move to the next character
        j parseFractional      # Loop back

endFractional:
        sw $t2, fracPart # Store the fractional part
        sw $t1, TENpower          # Store the TENpower #t3
        
        jr $ra                 # Return

# Function to convert the parts to floating point
...
convertToFlaot:
        ...
        # Load and convert TENpower
        lw $s0, TENpower
        mtc1 $s0, $f3
        cvt.s.w $f3, $f3

        # Divide fractional part by TENpower
        div.s $f2, $f2, $f3
        ...
        no_value_to_parsee: # Check for null terminator:
        jr $ra                 # Return
	
############################################################################################
# End of Function to parse a string to a float
############################################################################################
...
searchUnormal_by_test_name: 



               #the user enters the number of the test name
                   #promt
                    li $v0, 4
                    la $a0, prompt
                    syscall
		...
		enter_valid_optioSearch:
    				# Get user input
       				li $v0, 12          # syscall code for reading character
    				syscall
    				
    				move $a1,$v0 # save the user choice in a1
				...
				move $v0,$a1
                    #### move the content from a1 back to v0

                    li $t1, 49
                    beq $v0, $t1, Hgb_test_label_test_value_unnormal

                    li $t1, 50
                    beq $v0, $t1, BGT_test_label_test_value_unnormal

                    li $t1, 51
                    beq $v0, $t1, LDL_test_label_test_value_unnormal

                    li $t1, 52
                    beq $v0, $t1, BPT_test_label_test_value_unnormal

                  
                    # If invalid option, show error and go back to menu
                    li $v0, 4
                    la $a0, invalid_choice_msg_for_test_type
                    syscall
                    
                    j enter_valid_optioSearch


                    Hgb_test_label_test_value_unnormal:
                    li $t1, 0x111        # Hgb is 0x111
                    sw $t1, user_entered_test_name
                    j check_UNnormal_test_name
                          
                    
                    BGT_test_label_test_value_unnormal:
                    li $t1, 0xDD         # BGT is 0xDD
                    sw $t1, user_entered_test_name
                    j check_UNnormal_test_name
                           
                    LDL_test_label_test_value_unnormal: 
                    li $t1, 0xDC         # LDL is 0xDC
                    sw $t1, user_entered_test_name
                    j check_UNnormal_test_name
                    
                    BPT_test_label_test_value_unnormal:
                    li $t1, 0xE6         # BPT is 0xE6
                    sw $t1, user_entered_test_name
                          
             check_UNnormal_test_name: 
                   
                
                la $a0, buffer # Load the address of the buffer into $a0
                li $s7, 0 # Initialize the flag to 0 to indicate that the file is at start 
            ...
            CHECKFILE:
                 ...
                 li $t6, 0 # reset the value of t6 to 0 
                 jal testnamesEqual # this function returns in t6 1 if test names. are equal and 0 if not
                 beq $t6, 1, ABNORMALprinting 
                 # iff t6 = 0 mean the test name is not equal to the user_entered_test_name 
                 #if t6 = 1 mean the test name is equal to the user_entered_test_name
            ...
            move $a0, $t9          # Load the address of the start of the line into $a0
            jal line_by_line_function # f1 = test result in floating point
            # t4 = type of test, a0 = start of the next line

            ################################check boundries of tests ########################
            
            beq $t4, 1, HgbResult_unnormal_test_name
            beq $t4, 2, BGTResult_unnormal_test_name
            beq $t4, 3, LDLResult_unnormal_test_name
            beq $t4, 4, BPTResult_unnormal_test_name


            HgbResult_unnormal_test_name:

                            lwc1 $f3, HgbLowerBound # Load the lower bound value, which is 13.8
                            lwc1 $f4, HgbUpperBound # Load the upper bound value, which is 17.2
                            ...
                            j CHECKFILE       # Continue to check file IDs							
                                                        
                                    
                    
            BGTResult_unnormal_test_name:

                           lwc1 $f3, BGTLowerBound # Load the lower bound value is 70.0
                           lwc1 $f4, BGTUpperBound # Load the upper bound value is 99.0
            ...
            BPTResult_unnormal_test_name: 

                            lwc1 $f4, SystolicBPT # Load the upper bound value is 120.0
                            lwc1 $f3, DiastolicBPT # Load the upper bound value is 80.0
              ...
              printIfUnnormalByTestName:
                            ...
                            #store in found_bool the value 1 to indicate that the test result is found
                            li $t1, 1
                            sw $t1, found_bool
        ...
        check_founded_test_name_unnormal:
            ...
            #if the found_bool = 1 mean the test result is found and updated
        ...
        test_result_unnormal_by_test_name:
                ...
                #reset the value of found_bool to 0
                li $t1, 0
                sw $t1, found_bool
...
testnamesEqual:
            ...
            #load the value of the test name to check user_entered_test_name to s0

            find_semicolon_for_test_name_check:

                lb $t0, 0($a0)        # Load the next character from the input string into $t0
                beq $t0, '\0', no_more_lines_tocheck # Check for the end of the string
                ...
                lb $t0, 0($a0)        # Load the next character from the input string into $t0
                beq $t0, ' ', TestnameASCIsum # Check for the end of the string
                
                # sum the ascii values of the test name to choose the test value to calculate the average
                beq $t0, ',', testname_determiningg # If colon, have unique value for each test name 
                addu $t3, $t3, $t0     # Add the ASCII value to the sum for test name comparison
                    
                jal TestnameASCIsum

                testname_determiningg:

                    # Check the sum of the ASCII values to determine the test name
                    li $t1, 0x111        # ASCII sum for "Hgb"
                    beq $t3, $t1, HgbResult_check # If the sum matches "Hgb", jump to HgbResult

                    li $t1, 0xDD         #  ASCII sum for "BGT"
                    beq $t3, $t1, BGTResult_check # If the sum matches "BGT", jump to BGTResult

                    li $t1, 0xDC        #  ASCII sum for "LDL"
                    beq $t3, $t1, LDLResult_check # If the sum matches "LDL", jump to LDLResult

                    li $t1, 0xE6         #  ASCII sum for "BPT"
                    beq $t3, $t1, BPTResult_check # If the sum matches "BPT", jump to BPTResult

                #return unique value for each test name

                    HgbResult_check:
                    la $a0, user_entered_test_name   # Load address of the value of the test result into $a0
                    lw $s0 , 0($a0) # load the value of the test result to s0
                    # compare the test name with the user_entered_test_name
                    #then return to ra in t8
                    beq $s0,$t1 , equal_test_name # If the test name matches the user_entered_test_name, return 1
                    ...
                    equal_test_name:
                    li $t6, 1
                    move $ra, $t8 # restore the return addresfrom t8
                    jr $ra

                    no_more_lines_tocheck:
                    li $t6, 0 #reset the value of t6 to 0
                    move $ra, $t8 # restore the return address from t8
                    li $s7 , 1 # set the value of s7 to 1 to indicate that the buffer is done.
                    jr $ra
